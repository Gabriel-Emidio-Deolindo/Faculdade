...
function entrar(event) {
  ...
  area.innerHTML =
    '<span class="prompt">$</span> acesso liberado: ' +
    nome +
    ' <span class="tag">[' +
    curso +
...
function sair() {
  area.innerHTML =
    '<span class="prompt">$</span> sessão encerrada<span class="cursor">_</span>';
  ...
  setTimeout(function () {
    area.innerHTML =
      '<span class="prompt">$</span> aguardando login<span class="cursor">_</span>';
  }, 1400);
}
...
function calcularMedia(event) {
  ...
  if (media >= 6) {
    resultado.innerHTML =
      '<span class="linha-log">&gt; média calculada: ' +
      media.toFixed(2) +
      "</span>" +
